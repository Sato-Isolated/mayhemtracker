    ...
    <div data-testid={`match-history-row-${match.matchId}`}>
      ...
      <div className={`grid transition-[grid-template-rows] duration-200 ${isExpanded ? "grid-rows-[1fr]" : "grid-rows-[0fr]"}`}>
        <div className="overflow-hidden">
          {isExpanded ? (
            <div id={`match-history-inline-${match.matchId}`} className="mx-2 rounded-b-[1.35rem] border border-t-0 border-border/70 bg-card/76 p-4 shadow-[inset_0_1px_0_color-mix(in_oklch,var(--border)_55%,transparent)]" data-testid={`match-history-inline-${match.matchId}`}>
              {detail ? (
